...
/**
 * Deterministic generator. Uses the proposal’s type, client and title.
 * Swap this file for a network provider later — the registry stays the same.
 */
export function generateDeterministicImprovement({ finding, proposal } = {}) {
  const issue = finding ?? {}
...
registerImproveProvider({
  ...
  label: 'Deterministic',
  generate: generateDeterministicImprovement,
})
